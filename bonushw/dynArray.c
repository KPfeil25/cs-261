...
void addHeap(DynArr *heap, TYPE node)
{
	...
	addDynArr(heap, node);
	currIndex = (sizeDynArr(heap) - 1);
	parentIndex = (currIndex - 1) / 2;		/* solve for the index of the parent node */
	parentValue = heap->data[parentIndex];	/* set the value */

	while (currIndex > 0 && compare(node, parentValue) == -1) { /* adds as long as the index is on the array and then percolates until the correct spot is found in relation to parent node */
		swapDynArr(heap, currIndex, parentIndex);
		currIndex = parentIndex;
		parentIndex = ((currIndex - 1) / 2); /* re calculate new parents index */
...
{
  	int l, r, min;
	l = ((2*pos) + 1);	/* solve for the left and right children indecies */
	r = ((2*pos) + 2);

	if (r <= max) {
		min = _smallerIndexHeap(heap, l, r);
			if (heap->data[min].priority < heap->data[pos].priority) { /* adjust based on the parent/child nodes not fitting the format of the min-heap */
				swapDynArr(heap, min, pos);
				_adjustHeap(heap, max, min);
			}
	}
	else if (l <= max) {
		if (heap->data[l].priority < heap->data[pos].priority) {
			swapDynArr(heap, l, pos);
			_adjustHeap(heap, max, l);
		}
	}
}
...
{
	heap->data[0] = getDynArr(heap, (sizeDynArr(heap) - 1)); /* we know becuase this is a min-heap that the minimum value will be at index 0 */
	heap->size--; /* bookeep size */
...
void _buildHeap(DynArr *heap)
{
	...
	for (index = ((max_idx - 1) / 2); index >= 0; index--) {
		_adjustHeap(heap, max_idx, index);
	}
}
/* 
...
void sortHeap(DynArr *heap)
{
	int max_idx;
	_buildHeap(heap);
	for (max_idx = (heap->size - 1); max_idx >= 0; max_idx--) {
		swapDynArr(heap, max_idx, 0);
		_adjustHeap(heap, max_idx - 1, 0);
	}
}
